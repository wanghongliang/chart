	...
	var ScaleClass = Chart.Scale.extend({
		...
		draw : function(){
			...
			if (this.display){
				...
				//this.volumeStartPoint Y坐标的开始位置
				//this.volumeEndPoint Y坐标的结束位置
				//this.width-xStart 宽度
				//this.volumeEndPoint-this.volumeStartPoint 高度
				//ctx.strokeRect( xStart, this.volumeStartPoint, this.width-xStart, this.volumeEndPoint-this.volumeStartPoint);//绘制矩形轮廓
	...
	Chart.Type.extend({
		...
		initialize:  function(data){
			...
			helpers.each(data.datasets,function(dataset){ 
				...
				//alert( "数组长度:" + datasetObject.volPoints.length );
				//if( b == false ){
				//	b = true;
		...
		draw : function(ease){
			...
			helpers.each(this.datasets,function(dataset){
				...
				if( dataset.avgVolume5 ){ 
					...
					ctx.textAlign		= "left";
					ctx.textBaseline	= "middle"; 
					ctx.fillText( "5日均量", this.scale.width+2, linePositionY );
		...
		showTooltip : function(ChartElements, pos){
			...
			if( ChartElements.length>0){ 
				...
				if( this.volPoints[point.label] ){
					//显示当前的价格
					ctx.fillText( "价格: "+point.value, leftRange, 8 );  
					ctx.fillText( "涨跌幅: "+ (point.value /this.scale.centerValue*100-100).toFixed(2) + "%", leftRange+70, 8 );  
					ctx.fillText( "交易量: "+this.volPoints[point.label].value, leftRange+160, 8 );  
				}
